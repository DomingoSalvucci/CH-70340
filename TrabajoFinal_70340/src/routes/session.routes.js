...
sessionRouter.get("/current", authentication, (req, res) => {
  if (!res.cookies.tokenCookie) return res.status(401).json({ message: "Unauthorized" })
  res.status(200).json({message: "Current User logged in", user: req.cookies.tokenCookie})
})
